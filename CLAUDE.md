...
- **垃圾信定義與安全名單**詳見 `.claude/commands/cleanup-junk.md`：採「**寄件者黑名單**」精準清理，已讀與未讀都處理。
- **核心安全原則**：只刪黑名單上的寄件者；銀行帳單、工作/試驗/研究、帳號安全、收據、本人帳號等**一律保留**，寧可漏刪、不可誤刪。
- 「刪除」＝移到垃圾桶（30 天內可救回），不是永久刪除。
- 發現黑名單未涵蓋的新垃圾來源時，**先列給我確認**再加入名單，不擅自擴大範圍。
- 已設定每日 07:33 自動執行 `/cleanup-junk`。
...
| `/cleanup-junk` | 清理垃圾信（已讀＋未讀，依寄件者黑名單移到垃圾桶）|
| `/remind` | 設定一次性或週期性提醒 |
| `/agenda` | 查詢今天／本週行程 |
